...
- `scripts/notify-weixin.mjs` is the Weixin-only compatibility sender. Completion notifications are sent as plain text with readable line breaks and separators.
...
- `scripts/weixin-command-router.mjs` sends outbound replies as plain text, preserving line breaks and converting horizontal rules to separators.
- `task snap` / `task screenshot` / `任务 截图` returns the current task's tmux pane as formatted plain text.
...
- `task snap`, `task screenshot`, `任务 截图`, `截图`: return the current task's tmux pane as formatted plain text.
...
- Ordinary task messages and attachment messages send a first text heartbeat before Codex processing. In interactive mode, no second acknowledgement is sent after dispatch; completion output and interactive questions still arrive normally. `task snap` / `任务 截图` sends `task N · 查看中` first. Immediate commands such as `list`, `列表`, `task N`, `任务 N`, `task close ...`, `任务 关闭 ...`, and alias commands do not send a separate heartbeat.
- Interactive task replies use a background watcher so the router stays responsive; it sends choice prompts immediately and sends the final plain-text result when Codex returns to the input prompt or prints `Worked`. Watcher state is stored in task metadata so a router restart with `--no-restart-tasks` can resume waiting. `interactiveResponseTimeoutMs` is only an abnormal timeout and defaults to 21600000 ms; status pings default to every 1800000 ms and can be overridden with `interactiveWatchStatusIntervalMs` or `CODEX_WEIXIN_INTERACTIVE_WATCH_STATUS_INTERVAL_MS`.
...
- `task snap` / `任务 截图` returns a static text output only; the user still controls the task by sending normal Weixin text replies.
- Normal replies and completion notifications are plain text. Line breaks are preserved, repeated blank lines are collapsed, and horizontal rules become readable separators. Explicit local media directives remain supported.
