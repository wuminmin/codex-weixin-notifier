...
Agents call `sync_session` at work start, around major stages, after verification, before completion, and about every five minutes. Call `notify_author` whenever a proactive progress update, warning, or other useful message should reach the author. It is non-idempotent and may be called any number of times per `work_cycle_id`; every call sends a separate notification without changing the session status. When author input is required, ask in the active agent conversation. Before the final answer, the agent drafts the exact complete user-visible response, calls `notify_work_completed` with that response unchanged in `summary`, and then sends the same response to the user without edits. CATM prepends an identity header containing the agent type, session id, work-cycle id, workspace, and task label. `verification` remains stored internal metadata and is not rendered in the author notification.

Verify that the remote MCP catalog contains only `sync_session`, `notify_author`, and `notify_work_completed`; that repeated `notify_author` calls each reach every enabled target; that a completion reaches every enabled target; and that ordinary inbound text produces no reply or stored instruction.
