...
import { fanOutAuthorMessage, formatCompletionMessage } from "./channel-notifier.mjs";

const INSTRUCTIONS = `CATM is a notification-only service for coding-agent sessions. Call sync_session when work begins, after major stages, after verification, before completion, and about every five minutes during continuous work. Keep the returned session_id and work_cycle_id for this conversation. CATM does not collect author decisions or remote instructions; ask the author in the active agent conversation whenever input is required. Never send credentials, secrets, full logs, or unnecessary source code. Before final delivery, draft the exact complete user-visible final response, call notify_work_completed exactly once for the current work cycle with that response unchanged in summary, then send the same response to the user without edits. CATM adds the agent identity header; verification is internal metadata and is not rendered in the author notification.`;
